...
echo Associate the Local subnet route table
...
--output text create-network-interface --subnet-id $LZ_SUBNET_ID \
...
--tag-specifications 'ResourceType=instance,Tags=[{Key="kubernetes.io/cluster/localzone-sno",Value=shared}]') \
